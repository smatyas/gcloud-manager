...
class InstancesStartCommand extends AbstractCommand
{
    protected function configure()
    {
        $this
            ->setName('compute:instances:start')
            ->setDescription('Starts the compute instances.')
            ->addArgument(
                'instances',
                InputArgument::IS_ARRAY | InputArgument::REQUIRED,
                'The name of the instances to start.'
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($instances as $instance) {
            $operation = $computeService->instances->start($project, $zone, $instance);
